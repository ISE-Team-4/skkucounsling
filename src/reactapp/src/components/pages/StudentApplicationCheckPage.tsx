...
const StudentApplyPage = observer(() => {
  ...
  useEffect(() => {
    setFileContent(studentStore.applicationContent);
  }, [studentStore.applicationContent]);
  ...
  // 신청서 양식 다운로드
  const handleDownload = (saveName: string, content: string) => {
    // 파일을 읽어오는 동기 함수
    ...
    return (
      ...
      >
        ...
        <Text fontSize="md" fontWeight="400" textAlign={"left"}>
          ...
          {appliedAt?.getDate()}일 ({day[appliedAt.getDay()]})
        </Text>
        <Text fontSize="md" fontWeight="400" textAlign={"left"}>
        ...
        <Text fontSize="md" fontWeight="400" textAlign={"left"}>
          ...
          {day[expectedDate.getDay()]})
        </Text>
        <Text fontSize="sm" fontWeight="400" textAlign={"left"}>
          (*상담 예정일은 실제와 다를 수 있습니다. 심리 상담 후 상담일이
          확정됩니다.)
        </Text>
      </VStack>
    );
  ...
  const CounselingType = observer(() => {
    return (
      ...
      >
        ...
        >
          <Stack>
            <Radio colorScheme="green" value="personal_1" isDisabled>
              검사 해석 상담(검사 후 1회기 해석)
            </Radio>
            <Radio colorScheme="green" value="personal_5" isDisabled>
              5회기 개인 상담
            </Radio>
            <Radio colorScheme="green" value="personal_10" isDisabled>
              10회기 개인 상담
            </Radio>
          </Stack>
        </RadioGroup>
      ...
      >
        ...
        <CheckboxGroup colorScheme="green" value={fields}>
          <Stack
            ...
            style={{ margin: "1rem" }}
          >
            <Checkbox value="대인관계" disabled>
              대인관계
            </Checkbox>
            <Checkbox value="성격 및 적응" disabled>
              성격 및 적응
            </Checkbox>
            <Checkbox value="학업 및 진로" disabled>
              학업 및 진로
            </Checkbox>
            <Checkbox value="심리 및 정서" disabled>
              심리 및 정서
            </Checkbox>
            <Checkbox value="가족 관계" disabled>
              가족 관계
            </Checkbox>
            <Checkbox value="결혼 및 연애" disabled>
              결혼 및 연애
            </Checkbox>
            <Checkbox value="종교 및 가치관" disabled>
              종교 및 가치관
            </Checkbox>
          </Stack>
        </CheckboxGroup>
  ...
  const FormDownloader = observer(() => {
    return (
      ...
      >
        ...
        <Button
          onClick={() => studentStore.fetchApplicationFile()}
          size="sm"
